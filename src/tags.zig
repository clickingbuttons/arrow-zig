...
pub const Tag = union(enum) {
	...
	pub fn size(self: Self) usize {
		return switch (self) {
			...
			.Binary => 1,
			.FixedBinary => |b| @intCast(usize, b.fixed_len),
			.Dictionary => |d| switch (d.index) {
				.i8 => 1,
				.i16 => 2,
				.i32 => 4,
	...
	pub fn offsetSize(self: Self) usize {
		return switch (self) {
			...
			.FixedList => |f| if (f.large) 8 else 4,
			.Union => |u| if (u.dense) 4 else 0,
			else => 0
		};
	}

	fn fromAbiFormat2(format: []const u8, is_nullable: bool, is_dictionary: bool) !Self {
		...
		switch (try reader.readByte()) {
			...
			'u' => return .{ .Binary = .{ .large = false, .utf8 = true } },
			'U' => return .{ .Binary = .{ .large = true, .utf8 = true } },
			'z' => return .{ .Binary = .{ .large = false, .utf8 = false } },
			'Z' => return .{ .Binary = .{ .large = true, .utf8 = false } },
			'd' => {
			...
			't' => {
				switch (try reader.readByte()) {
					...
					's' => {
						switch (try reader.readByte()) {
							's' => return .{ .Timestamp = .{ .nullable = is_nullable, .unit = .second, .timezone = format[4..] } },
							'm' => return .{ .Timestamp = .{ .nullable = is_nullable, .unit = .millisecond, .timezone = format[4..] } },
							'u' => return .{ .Timestamp = .{ .nullable = is_nullable, .unit = .microsecond, .timezone = format[4..] } },
							'n' => return .{ .Timestamp = .{ .nullable = is_nullable, .unit = .nanosecond, .timezone = format[4..] } },
							else => {},
						}
					},
					else => {},
	...
	pub fn fromAbiFormat(format: []const u8, is_nullable: bool, is_dictionary: bool) !Self {
